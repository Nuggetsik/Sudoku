...
void draw_area() {
    ALLEGRO_COLOR gridColor = al_map_rgb(0, 0, 0); // Цвет сетки
    ALLEGRO_COLOR sectionColor = al_map_rgb(128, 128, 128); // Цвет секций

    // Рисуем вертикальные линии
    for (int i = 0; i <= 9; ++i) {
        int x = i * (W / 3.75); // Координата X текущей линии
        if (i % 3 == 0) {
            al_draw_line(x/3, 0, x/3, H/ 1.26, gridColor, 6.0); // Рисуем более толстые линии для секций
        }
        else {
    ...
    for (int i = 0; i <= 9; ++i) {
        int y = i * (H / 3.8); // Координата Y текущей линии
        if (i % 3 == 0) {
            al_draw_line(0, y/3, W/ 1.25, y/3, gridColor, 6.0); // Рисуем более толстые линии для секций
        }
        else {
